...
export const PlayerProvider: React.FC<{ children: React.ReactNode }> = ({
    ...
    const [playingNext, setPlayingNext] = useState<AzuraPlayingNext | null>(
        null,
    );
    ...
    useEffect(() => {
        ...
        const update = async () => {
            try {
                ...
                const data: AzuraNowPlayingResponse =
                    await fetchNowPlayingFromBackend();
                if (!active) return;
                ...
                if (data.station.listen_url) {
                    setStreamUrl((prev) => {
                        if (prev !== data.station.listen_url)
                ...
                console.error(
                    "Failed to fetch now playing from backend, using fallback:",
                    err,
                );
