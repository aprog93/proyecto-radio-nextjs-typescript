/**
 * Typed API Client for Radio Comunitaria Cesar Backend
 ...
 * 2. { success: true, events: [...], total: n } - direct (our backend)
 */
async function unwrapResponse<T>(response: ApiResponse<T>): Promise<T> {
    ...
    // If response has direct data (our backend format), return it
    if (response.data === undefined) {
        // For responses like { success: true, events: [...], total: n }
...
export const blogs = {
    getPublished: async (
        ...
        const response = await apiCall<{ blogs: Blog[]; total: number }>(
            `/blogs?${params}`,
        );
        return unwrapResponse(response);
    },
...
export const news = {
    getPublished: async (
        ...
        if (search) params.append("search", search);
        const response = await apiCall<{ news: News[]; total: number }>(
            `/news?${params}`,
        );
        return unwrapResponse(response);
    },
...
export const events = {
    getPublished: async (
        ...
        if (search) params.append("search", search);
        const response = await apiCall<{ events: Event[]; total: number }>(
            `/events?${params}`,
        );
        return unwrapResponse(response);
    },
    ...
    register: async (eventId: number, token?: string): Promise<void> => {
        ...
        if (!response.success) {
            throw new Error(
                response.error?.message || "Failed to register for event",
            );
        }
    },
    ...
    unregister: async (eventId: number, token?: string): Promise<void> => {
        ...
        if (!response.success) {
            throw new Error(
                response.error?.message || "Failed to unregister from event",
            );
        }
    },
};
...
export const products = {
    getPublished: async (
        ...
        if (category) params.append("category", category);
        if (search) params.append("search", search);
        const response = await apiCall<{ products: Product[]; total: number }>(
            `/products?${params}`,
        );
        return unwrapResponse(response);
    },
...
export const admin = {
    listUsers: async (
        ...
        if (search) params.append("search", search);
        const response = await apiCall<{ data: User[]; total: number }>(
            `/admin/users?${params}`,
            {},
            finalToken,
        );
...
export const schedule = {
    ...
    getByDay: async (dayOfWeek: number): Promise<ScheduleEvent[]> => {
        const response = await apiCall<ScheduleEvent[]>(
    ...
    delete: async (id: number, token?: string): Promise<void> => {
        ...
        if (!response.success) {
            throw new Error(
                response.error?.message || "Failed to delete schedule",
            );
        }
    },
};
